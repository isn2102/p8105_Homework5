...
Create two new datasets for all mean estimates and only rejected mean estimates, then plot both on the same graph. 
```{r}
sim_results %>% 
unnest(data) %>% 
select(-p.value) %>% 
ggplot(aes(x = mean_val, y = estimate, color = rejected)) +
geom_point() +
xlab("True Mean Value") +
...
**Change number of simulations to 5000**
